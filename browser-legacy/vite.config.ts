  ...
  ssgOptions: {
    ...
    onPageRendered: async (route, renderedHTML) => {
      ...
      if (route.startsWith('/resolution/') && title) {
        const fullTitle = `${title} | ISO/TC 184/SC 4`
        html = html.replace(/<title>.*?<\/title>/, `<title>${fullTitle}</title>`)

        const descMatch = html.match(/<p[^>]*class="[^"]*res-detail-subtitle[^"]*"[^>]*>(.*?)<\/p>/s)
        const desc = descMatch ? descMatch[1].replace(/<[^>]*>/g, '').trim() : `${title} — ISO/TC 184/SC 4 resolution.`
        ...
        const fullTitle = `Meeting: ${title} | ISO/TC 184/SC 4`
        html = html.replace(/<title>.*?<\/title>/, `<title>${fullTitle}</title>`)
        html = html.replace('</head>', `<meta name="description" content="Resolutions from ${title} — ISO/TC 184/SC 4">\n  <meta property="og:title" content="${fullTitle.replace(/"/g, '&quot;')}">\n  <meta property="og:description" content="Resolutions adopted at ${title.replace(/"/g, '&quot;')}">\n  <meta property="og:type" content="website">\n</head>`)
      } else if (route === '/meetings') {
        html = html.replace(/<title>.*?<\/title>/, '<title>Meetings | ISO/TC 184/SC 4</title>')
        html = html.replace('</head>', `<meta name="description" content="Browse ISO/TC 184/SC 4 plenary meetings by year, country, and venue.">\n  <meta property="og:title" content="Meetings | ISO/TC 184/SC 4">\n  <meta property="og:description" content="Browse ISO/TC 184/SC 4 plenary meetings by year, country, and venue.">\n</head>`)
      } else if (route === '/about') {
        html = html.replace(/<title>.*?<\/title>/, '<title>About | ISO/TC 184/SC 4</title>')
        html = html.replace('</head>', `<meta name="description" content="About the ISO/TC 184/SC 4 resolutions database.">\n</head>`)
      } else if (route === '/') {
        const countMatch = html.match(/(\d[\d,]*)\s+resolutions?/i)
        const count = countMatch ? countMatch[1] : ''
        const descSuffix = count ? `${count} resolutions of` : 'resolutions of'
        html = html.replace('</head>', `<meta name="description" content="Search and browse ${descSuffix} ISO/TC 184/SC 4 — Industrial data.">\n  <meta property="og:title" content="ISO/TC 184/SC 4 Resolutions">\n  <meta property="og:description" content="Search and browse resolutions of ISO/TC 184/SC 4 — Industrial data.">\n</head>`)
      }
